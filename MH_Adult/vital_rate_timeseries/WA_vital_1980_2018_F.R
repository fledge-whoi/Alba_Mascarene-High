...
IV <- initfct(jagsdata = jagsdata, perfect_detection = F)  # Run script 202005_fctinits.r before 


# No individual should be marked at the last occasion (those individuals are useless anyway)
# All marked individuals are juvenile

#AGE=matrix(NA, nrow=dim(jagsdata$DETECTED)[1],ncol=dim(jagsdata$DETECTED)[2])

#for(i in 1:dim(jagsdata$DETECTED)[1]){
#  AGE[i,jagsdata$FIRST[i]] <- 0
#  if(jagsdata$LAST[i]>jagsdata$FIRST[i]){
#    for(j in (jagsdata$FIRST[i]+1):dim(jagsdata$DETECTED)[2]){
#      AGE[i,j]= AGE[i,(j-1)]+1
#    }
#  }
#}
...
parameters <- c(#female mean vital rates
                "mean.Fphi_SB","mean.Fphi_FB","mean.Fphi_PSB","mean.Fphi_PFB","mean.Fphi_NB",
                "mean.Frho_SB","mean.Frho_FB","mean.Frho_PSB","mean.Frho_PFB","mean.Frho_NB",
                "mean.Fpi_SB","mean.Fpi_FB","mean.Fpi_PSB","mean.Fpi_PFB","mean.Fpi_NB",
                "epsilon_s", "epsilon_b", "epsilon_bs"
                )
# MCMC settings
...
out1 <- jags(jags.data, inits, parameters, "WA_vital_1980_2018_F.txt", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

save(out1,file = "WA_vital_1980_2018_F.RData")
