...
DETECTED<-DETECTED[which(SEX==1),]
SURVIVAL<-SURVIVAL[which(SEX==1),]
BREED<-BREED[which(SEX==1),]
OFFSPRING<-OFFSPRING[which(SEX==1),]
SCORE <- SCORE[which(SEX==1)]
AGE <- AGE[which(SEX==1),]
...
# Bundle data
jags.data <- list(SURVIVAL = jagsdata$SURVIVAL, BREED = jagsdata$BREED, OFFSPRING = jagsdata$OFFSPRING, DETECTED = jagsdata$DETECTED, AGE = jagsdata$AGE, 
...
parameters <- c(#male mean vital rates
                 #breeding 
                "mean.Mphi_SB","mean.Mphi_FB","mean.Mphi_PSB","mean.Mphi_PFB","mean.Mphi_NB",
                "mean.Mrho_SB","mean.Mrho_FB","mean.Mrho_PSB","mean.Mrho_PFB","mean.Mrho_NB",
                "mean.Mpi_SB","mean.Mpi_FB","mean.Mpi_PSB","mean.Mpi_PFB","mean.Mpi_NB",
...
out1 <- jags(jags.data, inits, parameters, "WA_vital_1980_2018_M.txt", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

save(out1,file = "WA_vital_1980_2018_M.RData")
